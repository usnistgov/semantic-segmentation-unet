...
% Derek Comment: "Although the decoupling of training and inference size could allow models to be trained more quickly, it seems neither necessary nor sufficient for applying the models to images larger than GPU memory.  It is not necessary because, as long as an out-of-core technique were developed, images of the full inference size could be used for training.  It is not sufficient because, if an out-of-core technique has not been developed, images larger than GPU memory could still not be inferenced.  The ability to apply the model to images greater than GPU memory depends only on an out-of-core technique, not on decoupling training and inference size."
...
%A convolutional layer (1) convolves a kernel and its input feature maps to create a set of output feature maps, (2) applies an element-wise a non-linearity (ReLu \citep{lecun2015deep}), and (3) performs batch normalization \citep{ioffe2015batch} on its output feature maps.
