...
\pdfinfo{
	/Title (Error-Free Semantic Segmentation Inference of Images Larger than GPU Memory)
%	/Author (Michael Majurski, Peter Bajcsy)
} %Leave this	
% /Title ()
...
%The U-Net architecture is an encoder-decoder type CNN network with skip connections at equivalent feature map sizes \cite{Ronneberger2015a}. Models like U-Net are a special type of CNN which only uses convolutional layers. 
...
%At the original U-Net output, the feature maps have shrunk down to $388 \times 388$ pixels.
...
%The input image to U-Net flows through the encoder, reaches the bottleneck feature map, and then flows through the decoder to create an output image of identical spatial size as the input. 
%With the change to \texttt{SAME} convolutions, the input image creates an output image of identical spatial size.

%See Dumoulin and Visin \cite{Dumoulin2018} for a more detailed explanation of the math behind transposed convolution/up-conv/fractionally strided convolution. 
...
%A spatial resolution changing layer is any layer which modifies the width or height of its input tensor. There are two examples present in U-Net: max-pool and up-conv. Max-pool halves the spatial resolution. Up-conv doubles the spatial resolution. All feature maps at the same level share a spatial resolution (when using \texttt{SAME} convolutions).
...
%\begin{figure}[h!]
%	\begin{center}
%		\includegraphics[width=\linewidth]{figs/offset_conv.png}
%	\end{center}
%	\caption{Simplified 1D example of an addition kernel of size 3 being applied at an offset less than the kernel size, producing different results.}
%	\label{fig:offset}
%\end{figure}
...
%\begin{figure}[h!]
%	\begin{center}
%		\includegraphics[width=0.7\linewidth]{figs/reflection_padding.png}
%	\end{center}
%	\caption{Simplified 1D example of reflection padding (reflected through dotted line) causing a different stride pattern across a set of pixels. The altered stride prevents the tile-based processing from collaboratively building subregions of a single feature map.}
%	\label{fig:reflection-padding}
%\end{figure}
...
512 & 256 & 128 & 0.0 & 0.0 \\
%512 & 224 & 144 & 2.61e-8 & 0.0 \\
%512 & 192 & 160 & 2.86e-9 & 0.0 \\
\end{tabular}
\end{table}
...
%The modified U-Net is shown in Figure \ref{fig:unet-mod}. 
Following Equation \ref{eq:radius} or \ref{eq:radius2} for this modified U-Net model produces a required radius value of 66. 

%\begin{figure}[h!]
%	\begin{center}
%		\includegraphics[width=\linewidth]{figs/unet-mod.png}
%	\end{center}
%	\caption{Modified U-Net model architecture with $n_l = 3$ and $M = 3$.}
%	\label{fig:unet-mod}
%\end{figure}
...
%1032 & 872 & 80 & 1.34e-8  & 0.0 \\
%1032 & 856 & 88 & 1.14e-8  & 0.0 \\
%1032 & 840 & 96 & 1.34e-8  & 0.0 \\
		\end{tabular}
\end{table}
...
%The dataset for this study consisted of an H9 human embryonic stem cell line \cite{Thomson1998} was prepared and images were collected as previously described \cite{Bhadriraju2016}. The use of dataset was approved by NIST and carried out in accordance with the approved guidelines of the NIST Human Subjects Protection Office.
