...
def _inference_tiling(img, unet, tile_size):
    ...
    assert unet.RADIUS % unet.SIZE_FACTOR == 0
    zone_of_responsibility_size = tile_size - 2 * unet.RADIUS
    ...
    for i in range(0, height, zone_of_responsibility_size):
        for j in range(0, width, zone_of_responsibility_size):
            ...
            x_st = x_st_z - unet.RADIUS
            y_st = y_st_z - unet.RADIUS
            x_end = x_end_z + unet.RADIUS
            y_end = y_end_z + unet.RADIUS

            radius_pre_x = unet.RADIUS
            if x_st < 0:
                x_st = 0
                radius_pre_x = 0

            radius_pre_y = unet.RADIUS
            if y_st < 0:
                radius_pre_y = 0
                y_st = 0

            radius_post_x = unet.RADIUS
            if x_end > width:
                radius_post_x = 0
                x_end = width
                x_end_z = width

            radius_post_y = unet.RADIUS
            if y_end > height:
                radius_post_y = 0
                y_end = height
                y_end_z = height
