...
def _inference_tiling(img, unet, tile_size, radius):

    # Pad the input image in CPU memory to ensure its dimensions are multiples of the U-Net Size Factor
    pad_x = 0
    pad_y = 0
    if img.shape[0] % unet.SIZE_FACTOR != 0:
        pad_y = (unet.SIZE_FACTOR - img.shape[0] % unet.SIZE_FACTOR)
        print('image height needs to be a multiple of {}, padding with reflect'.format(unet.SIZE_FACTOR))
    if img.shape[1] % unet.SIZE_FACTOR != 0:
        pad_x = (unet.SIZE_FACTOR - img.shape[1] % unet.SIZE_FACTOR)
        print('image width needs to be a multiple of {}, padding with reflect'.format(unet.SIZE_FACTOR))
    ...
    softmax = np.zeros((height, width, unet.number_classes), dtype=np.float32)

    assert tile_size % unet.SIZE_FACTOR == 0
    assert radius % unet.SIZE_FACTOR == 0
    zone_of_responsibility_size = tile_size - 2 * radius

    keras_model = unet.get_keras_model()
    ...
    for i in range(0, height, zone_of_responsibility_size):
        for j in range(0, width, zone_of_responsibility_size):
            ...
            sm = keras_model(batch_data)  # model output defined in unet_model is softmax
            sm = np.squeeze(sm)
...
def _inference(img, unet):
    ...
    keras_model = unet.get_keras_model()
    softmax = keras_model(batch_data) # model output defined in unet_model is softmax
...
def inference(saved_model_filepath, image_folder, output_folder, output_folder_softmax, image_format, tile_size, radius):
    # create output filepath

    if not os.path.exists(output_folder):
        os.mkdir(output_folder)
    if not os.path.exists(output_folder_softmax):
    ...
    model = unet_type_model.UNet(restore_saved_model_filepath=saved_model_filepath)
    ...
    for i in range(len(img_filepath_list)):
        ...
        if tile_size > 0:
            segmented_mask, softmax = _inference_tiling(img, model, tile_size, radius)
        else:
